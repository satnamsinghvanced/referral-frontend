...
export const fetchDevices = async (params: { page: number; limit: number }): Promise<DeviceResponse> => {
  const response = await axios.get("/devices-tracking", { params });
  return response.data;
};

export const toggleDevice = async (id: string, toggle: boolean): Promise<Device> => {
