...
export const fetchInAppNotifications = async (): Promise<NotificationsListResponse> => {
  const response = await axios.get("/notifications/in-app");
  return response.data;
};

export const markNotificationsAsRead = async (notificationIds: string[]): Promise<any> => {
...
export const updateNotificationSettings = async (id: string, payload: UpdateNotificationPayload): Promise<NotificationSettingsResponse> => {
