...
export default function VisitMap() {
	...
	const fetchAndAddMarker = async (
		...
		lat: number,
		index: number,
		isDestination: boolean // Kept for label but not for color decision
	) => {
		...
		// --- UPDATED COLOR LOGIC ---
		// Start (index 0) should be Green.
		// All other points (stops/destination) should be Red.
			...
			{routeSummary && (
				<div className="absolute top-8 left-8 bg-white shadow-lg rounded-xl p-4 z-20 w-64">
					<h2 className="text-lg font-semibold mb-2">
						{isOptimized
							? "Optimized Route (Fastest)"
							: "Original Route (First Result)"}
					</h2>
					{routeSummary.error ? (
