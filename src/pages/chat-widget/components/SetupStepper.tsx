...
export default function SetupStepper({ steps, activeStep, setActiveStep }: SetupStepperProps) {
  ...
  return (
    <div className="">
      ...
      <div className="relative flex justify-between items-start w-full mt-10 mb-8 p-0 overflow-hidden">
        {steps.map((step, idx) => {
          ...
          return (
            <div
              key={idx}
              className={`flex flex-col  relative z-10 group cursor-pointer w-12 ${idx == 0 ? "items-start" : "items-center"} ${idx == steps.length - 1 ? "items-end" : "items-center"}`}
              onClick={() => setActiveStep(idx)}
            >
              {idx < steps.length - 1 && (
                <div
                  className={`absolute top-[22px] h-[2px] -z-10 transition-all duration-300
                    ${idx == 0 ? "left-[110%] w-[320%]" : idx == steps.length - 1 ? "left-[50%] w-[100%]" : "left-[110%] w-[320%]"} 
                    ${activeStep >= idx + 1 ? "bg-emerald-500" : "bg-default-200 dark:bg-default-100/50"}`}
                />
              )}
