...
export default function SetupStepper({ steps, activeStep, setActiveStep }: SetupStepperProps) {
  ...
  return (
    <div className="">
      ...
      <div className="relative flex justify-between items-start w-full mt-10 mb-8 p-0">
        <div className="absolute top-[22px] left-[22px] right-[22px] h-[2px] bg-default-200 dark:bg-default-100/50 -z-10">
          <div
        ...
        {steps.map((step, idx) => {
          ...
          return (
            <div
              key={idx}
              className={`flex flex-col relative z-10 group w-12 ${idx <= activeStep ? "cursor-pointer" : "cursor-default"} ${idx == 0 ? "items-start" : "items-center"} ${idx == steps.length - 1 ? "items-end" : "items-center"}`}
              onClick={() => {
                if (idx <= activeStep) {
                  setActiveStep(idx);
                }
