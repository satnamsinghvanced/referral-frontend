...
const SuperAdminSignIn = () => {
  ...
  const formik = useFormik<FormData>({
    ...
    onSubmit: async (values) => {
      ...
      setErrorMsg("");
      try {
        const res: any = await loginSuperAdmin({ email: values.email, password: values.password });
        ...
        setErrorMsg(error.response?.data?.message || error.message || "Invalid email or password");
      } finally {
        setLoading(false);
      }
    },
  ...
  const handleForgotPasswordSubmit = async (e: React.FormEvent) => {
    ...
    setForgotSuccessMsg("");
    try {
      const res: any = await forgotSuperAdminPassword({ email: forgotEmail.trim() });
      const message = res?.message || res?.data?.message || "Password reset link sent to your email successfully.";
      setForgotSuccessMsg(message);
    } catch (error: any) {
      setForgotErrorMsg(error.response?.data?.message || error.message || "Failed to send password reset link. Please try again.");
    } finally {
