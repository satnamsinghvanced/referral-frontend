...
const ReferralCard = ({
    ...
    return (
            ...
            <CardHeader className="justify-between">
                <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                    ...
                    <div className="flex gap-2">
                        ...
                        {applicationStatus && (
                            <span
                                className={`px-2 py-0.5 rounded-full text-xs border capitalize ${applicationStatus === 'new' ? 'bg-blue-100 text-blue-600 border-blue-300' :
                                    applicationStatus === 'contacted' ? 'bg-yellow-100 text-yellow-600 border-yellow-300' :
                                        applicationStatus === 'scheduled' ? 'bg-green-100 text-green-600 border-green-300' :
                                            applicationStatus === 'completed' ? 'bg-gray-100 text-gray-600 border-gray-300' :
                                                applicationStatus === 'cancelled' ? 'bg-red-100 text-red-600 border-red-300' :
      ...
      ${urgency === 'medium' ? 'bg-yellow-100 text-yellow-700 border-yellow-300' : ''}
      ${urgency === 'high' ? 'bg-red-100 text-red-700 border-red-300' : ''}`}
            ...
            <CardBody className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                <div>
                    <p>
                        {referringPatient && <><strong>Referring Patient:</strong> {referringPatient}</>}
                        {referringDoctor && <><strong>Referring Doctor:</strong> {referringDoctor}</>}

                    </p>
                    <p>
                        {relationship && <><strong>Relationship:</strong> {relationship} </>}
                        {practice && <><strong>Practice:</strong>{practice}</>}
                    </p>
                    <p>
